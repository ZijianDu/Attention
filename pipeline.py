...
class StableDiffusionPipelineWithViT(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        num_images_per_prompt: Optional[int] = 1,
        eta: float = 0.0,
        vit_input_size: List[int] = None,
        vit_input_mean: torch.Tensor = None,
        vit_input_std: torch.Tensor = None,
        layer_idx: int = None,
        head_idx: int = None,
        guidance_strength: float = None,
        guidance_range: List[int] = None,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # use the modified DDIM scheduler
                if guidance_range is not None and (i < guidance_range[0] or i >= guidance_range[1]):
                    guidance_strength = 0
                latents, attention_mean, entropy, images, oneiterationqkv = self.scheduler.step(self.vit, self.vae, noise_pred, t, latents, vit_input_size,
                vit_input_mean, vit_input_std, layer_idx, head_idx, guidance_strength, **extra_step_kwargs, return_dict=False)
                attention_mean_per_timestep.append(attention_mean)
                entropy_per_timestep.append(entropy)
                images_per_timestep.append(images.cpu().numpy())
                allqkv.append(oneiterationqkv)
